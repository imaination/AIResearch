...
def write_out_movie( movie_file, selected_frame_times, coordinate_list, out_filename='human_intruder_test.mp4' ):
    # load movie_file
    mov = mpy.VideoFileClip(movie_file, audio=False)
...
def main():
    ...
    movie_file = '../movies/human_intruders11.mp4'
    num_images = 120
    coordinate_list = detection(movie_file, num_images, output_inf_graph_pth, training_dir) #returns a list of coordinates
    mov = mpy.VideoFileClip(movie_file, audio=False)
    selected_frame_times = np.arange(0,mov.duration, mov.duration/num_images)
    write_out_movie( movie_file, selected_frame_times, coordinate_list )
